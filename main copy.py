...
def iniciar_juego():
    ...
    while not(sudoku_importado.esta_terminado(sudoku_mapa)):
        ...
        if movimiento_jugador == "borrar":
            ...
            while not(posicion[0] in indice_fila) or not(posicion[1] in indice_columna):
                valor_correcto = input('¡Error! Por favor elija una posición valida entre las que figuran en pantalla: ')
                posicion = valor_correcto.split(", ")

            for item_f in indice_fila:
                if posicion[0] == item_f:
                    posicion[0] = indice_fila.index(item_f)
            
            for item_c in indice_columna:
                if posicion[1] == item_c:
                    posicion[1] = indice_columna.index(item_c)
            ...
            while not(movimiento[0] in indice_fila) or not(movimiento[1] in indice_columna) or not(movimiento[2] in indice_columna):
                movimiento_correcto = input('¡Error! Por favor elija un valor entre 1 - 9 y una posición valida entre las que figuran en pantalla: ')
